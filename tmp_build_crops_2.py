...
if __name__ == "__main__":
    ...
    src_lookup = single_one_off_import_lyd_json.build_src_lookup([r"C:\Users\twak\Documents\architecture_net\windows_part3\log_part_3.txt",
    ...
    if False:
        ...
        for i in images:
            ...
            # print (src_info['src'])
            out = os.path.join (dataset_root, "metadata_winodw_labels_2", Path ( src_info['src'] ).with_suffix(".json") )
            os.makedirs(Path(out).parent, exist_ok=True)
            with open(out, 'w') as _:
                pass

    if False:
        # these are the files in our format
        for i in glob.glob(r'C:\Users\twak\Documents\architecture_net\windows_part3\labels_all_labelled\*.json'):
            src_info = src_lookup[Path(i).with_suffix(".jpg").name]
            out = os.path.join(dataset_root, "metadata_labels_2", Path(src_info['src']).with_suffix(".json"))
            print(src_info['src'])
