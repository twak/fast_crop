...
def toy():
    offset = 2
    feats_real = np.random.rand(100).reshape(-1, 1)
    feats_fake = np.random.rand(100).reshape(-1, 1) + offset
    feats_real[0] = offset
    feats_fake[0] = 1
...
if __name__ == '__main__':
    ...
    # print('recall:', recall)
        ...
        for i, ti in enumerate (dataloader):
            ...
            realism_score = ipr.realism(ti)
            results.append(( Path(dataloader.dataset.fnames[i]).name[:-4], realism_score))
            print('realism of image:', realism_score)
            # break

    results.sort(key=lambda x: x[1])

    print (results)
    with open("./split.txt", "w") as split:
        for name,score in results:
            print (name)
            split.write( name + "\n" )

    with open("./scores.txt", "w") as split:
        for name,score in results:
            print (name)
            split.write( f"{name} : {score}\n" )
